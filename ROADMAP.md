...
## 🤝 How we work
- **Workflow v2 (from Day 4):** Claude plans each increment (data contract + UI spec + test list) and reviews results on request; **ox-alpha implements, tests, runs the gate, and merges** — the green gate is trusted as the review. Pushes to GitHub stay gated by Khawa.
...
- [x] **Day 4 — Tasks / Assignments.** First two-table feature: tasks link optionally to courses and due dates. **Data:** schema v3 (`tasks` table; string `status` — booleans aren't valid IndexedDB keys); list reads `toArray()` + JS sort because optional indexed fields are absent on some rows (`orderBy` would silently drop undated/unassigned tasks — regression-tested); `TaskPatch` null-sentinel clears fields without writing present-but-undefined keys; `courseId` existence checked inside the write transaction; `deleteCourse` now UNASSIGNS its tasks in one transaction (user content is never destroyed as a side effect) with the count named in the confirm dialog; pure `src/lib/dueDates.ts` (rolling 7-day buckets, DST-safe calendar arithmetic, locale-independent labels). **UI:** bucketed sections Overdue → Today → Tomorrow → This week → Later → No date (danger emphasis on Overdue), native `<details>` Done section ordered by completion time, shared `Checkbox`/`Textarea` primitives promoted, one-error-banner pattern from Day 3 formalized. Gate: lint 0 · **180 tests** · tsc · build green; scripted browser check extended to Tasks flows (8/8). Merged to `main` @ `35d96c6`. *Deletion policy from Day 3 applied as specified.*
